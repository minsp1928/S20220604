	...
	<div class="row">
		...
		<div class="row">
			<div class="col-5 col-lg-3 offset-lg-2" >
				<img src="${pageContext.request.contextPath}/img/${member.user_photo}" class="rounded float-start" id="pic"> 
			</div>
			<div class="col-7 col-lg-4">
				<table class="table" id="memtbl">
					<tr><th>아이디</th><td>${member.user_id }</td>
				 	...
				 	<tr><th>총 주문 건수</th><td>${member.orders}건</td><th>총 주문금액</th>
				 		<td><fmt:formatNumber value="${member.orderamount  }" pattern="#,###"/>원</td></tr>
				 	<tr><th>이름</th><td>${member.user_name}</td></tr>
				 	<tr><th>주소</th><td colspan="3">${member.con_addr }</td></tr>	 	 
				 	<tr><th>연락처</th><td colspan="3">${member.con_tel }</td></tr>	 	 
				 	<tr><th>이메일</th><td colspan="3">${member.email }</td></tr>	 	 		 	 
				 	<tr><th>상태변경</th><td colspan="3"><button onclick="memberStatus()" id="btnStatus"  class="btn btn-outline-secondary">
				 		<c:choose>
				 			<c:when test="${member.m_level == 3}">이용정지</c:when>
				 			<c:otherwise>활동중</c:otherwise>
				 		</c:choose></button></td></tr>
				 	<tr><th>비밀번호 초기화</th><td colspan="3"><button onclick="passReset()"  class="btn btn-outline-secondary">초기화</button></td></tr>	 	 		 	 
				</table>
			</div>
			<div class="col=lg-3">
			</div>
